...
def load_images_labels_heatmap_spot(dir, N, args, mask_filter='_masks', image_filter='_label', heatmap_filter='_gaumap_all', foldername = 'newlabels', target_n_channels=None):
    ...
    for n in range(nimg):
        ...
        if heatmap.ndim == 2:
            heatmap = heatmap[..., np.newaxis]  # shape (H, W, 1)
        
        # Pad heatmap channels if target_n_channels provided
        if target_n_channels is not None:
            n_channels = heatmap.shape[-1]
            if n_channels < target_n_channels:
                # Zero-pad along channel axis
                pad_width = [(0,0), (0,0), (0, target_n_channels - n_channels)]
                heatmap = np.pad(heatmap, pad_width, mode='constant')
            elif n_channels > target_n_channels:
                io_logger.warning(f'Heatmap has {n_channels} channels, exceeding target {target_n_channels}. Truncating.')
                heatmap = heatmap[..., :target_n_channels]
...
def load_train_test_data(train_dir, N, args, test_dir=None, image_filter='_image', mask_filter='_label', heatmap_filter='_gaumap_all', foldername = 'newlabels'):
    ...
    for each_train_dir in train_dir_list:
        if each_train_dir.startswith('.') or not os.path.isdir(os.path.join(train_dir, each_train_dir)):
            continue
        train_subdir_list.append(os.path.join(train_dir, each_train_dir))
    
    train_subdir_list = natsorted(train_subdir_list)
    
    # Determine maximum number of heatmap channels across all samples
    max_channels = 0
    all_dirs = train_subdir_list.copy()
    if test_dir is not None:
        test_dir_list = os.listdir(test_dir)
        test_subdir_list = []
        for each_test_dir in test_dir_list:
            if each_test_dir.startswith('.') or not os.path.isdir(os.path.join(test_dir, each_test_dir)):
                continue
            test_subdir_list.append(os.path.join(test_dir, each_test_dir))
        all_dirs.extend(test_subdir_list)
    
    io_logger.info('Scanning heatmaps to determine channel count...')
    for dir_path in all_dirs:
        heatmap_names = get_heatmap_files(dir_path, heatmap_filter)
        for hm_path in heatmap_names:
            hm = imread(hm_path, 'heatmap')
            if hm is None:
                continue
            if hm.ndim == 2:
                channels = 1
    ...
    io_logger.info(f'Maximum heatmap channels found: {max_channels}')
    if max_channels == 0:
        max_channels = None  # fallback, will not pad
    ...
    for each_train_dir in train_subdir_list:
        images, labels, heatmaps, spots, label_names = load_images_labels_heatmap_spot(each_train_dir, N, args, mask_filter, 
                                                                            ...
                                                                            foldername, target_n_channels=max_channels)
    ...
    if test_dir is not None:
        ...
        for each_test_dir in test_subdir_list:
            test_images, test_labels, test_heatmaps, test_spots, test_label_names = load_images_labels_heatmap_spot(each_test_dir, N, args, mask_filter, image_filter, heatmap_filter, foldername, target_n_channels=max_channels)
