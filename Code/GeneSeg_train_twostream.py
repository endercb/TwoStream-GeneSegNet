...
training_args.add_argument('--cross_attn_layers', default='2,3', type=str, help='Comma-separated layer indices for cross-attention (default: 2,3)')
...
def label_postprocess(args, logger, N):            
    ...
    # Parse cross-attention layers
    cross_attn_layers = tuple(map(int, args.cross_attn_layers.split(',')))
    logger.info(f'>>>> Cross-attention layers: {cross_attn_layers}')
    ...
    for infer in inference_list:
        ...
        n_genes = heatmaps[0].shape[-1] if heatmaps[0].ndim == 3 else 1
        logger.info(f'>>>> Detected n_genes = {n_genes}')
        ...
        if args.all_channels:
            nchan = images[0].shape[-1] if images[0].ndim == 3 else 1
            channels = None 
        else:
            nchan = images[0].shape[-1] if images[0].ndim == 3 else 1
...
def train(args, logger, N):
    ...
    # Compute number of genes (heatmap channels)
    # heatmaps shape: each element (H, W, N) or (H, W)? Assume (H, W, N)
    n_genes = heatmaps[0].shape[-1] if heatmaps[0].ndim == 3 else 1
    logger.info(f'>>>> Detected n_genes = {n_genes}')
    # Concatenate DAPI (1 channel) and heatmaps (n_genes channels) along channel dimension
    images = list(np.concatenate((images, heatmaps), axis=3))
    ...
    if args.all_channels:
        img = images[0]
        if img.ndim==3:
            nchan = img.shape[-1]  # 1 + n_genes
        elif img.ndim==2:
            nchan = 1
        channels = None 
    else:
        # Override args.chan with actual total channels
        nchan = images[0].shape[-1] if images[0].ndim == 3 else 1 

    logger.info('>>>> during training rescaling images to fixed diameter of %0.1f pixels'%args.diam_mean)
    logger.info('>>>> START TRAINING')    
    
    model = TwoStreamGeneSegModel(device=device,
                                pretrained_model=pretrained_model,
                                diam_mean=args.diam_mean,
                                residual_on=args.residual_on,
                                style_on=args.style_on,
