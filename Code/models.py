...
import cv2
from two_stream_net import TwoStreamCPnet
from cross_attention import ChannelWiseCrossAttention, CrossAttention
...
class TwoStreamGeneSegModel(GeneSegModel):
    """
    GeneSegModel with two-stream encoder and cross-attention.
    Replaces CPnet with TwoStreamCPnet.
    """
    def __init__(self, gpu=False, pretrained_model=False,
                 diam_mean=30., net_avg=False, device=None,
                 residual_on=True, style_on=True, concatenation=False,
                 nclasses=3, nchan=2, n_genes=None, cross_attn_layers=(2,3)):
        ...
        # Determine number of genes if not provided (default mouse hippocampus)
        if n_genes is None:
            n_genes = nchan - 1  # assume nchan = 1 + n_genes
            if n_genes <= 0:
                n_genes = 33
                models_logger.warning(f'n_genes inferred as {n_genes}')
        
        # Two-stream channel bases
        nbase1 = [1, 32, 64, 128, 256]   # DAPI stream
        nbase2 = [n_genes, 32, 64, 128, 256]  # HeatMap_all stream
        self.nbase = nbase1  # for compatibility with parent class
        
        ostr = ['off', 'on']
        self.net_type = 'twostream_geneseg_{}_residual_{}_style_{}_concatenation_{}'.format(nclasses,
                                                                                         ostr[residual_on],
                                                                                         ostr[style_on],
                                                                                         ostr[concatenation])
        if pretrained_model:
            models_logger.info(f'net type: {self.net_type}')
        ...
        # Create TwoStreamCPnet (disable mkldnn to avoid OpaqueTensor issues)
        self.net = TwoStreamCPnet(nbase1, nbase2, nclasses, sz=3,
                                  residual_on=residual_on, style_on=style_on,
                                  concatenation=concatenation, mkldnn=False,
                                  diam_mean=diam_mean, cross_attn_layers=cross_attn_layers).to(self.device)
