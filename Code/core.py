...
#         slc = [slice(0, self.nclasses) for n in range(imgs.ndim)] # changed from imgs.shape[n]+1 for first slice size 
    ...
    def train(self, train_data, train_labels, train_files=None, 
        ...
        def get_mask(l):
            return l[0] if l.ndim == 3 and l.shape[0] >= 1 else l.squeeze()

        if self.nclasses==3:
            core_logger.info('computing boundary pixels for training data')
            train_classes = [np.stack((get_mask(label), get_mask(label)>0, utils.distance_to_boundary(get_mask(label))), axis=0).astype(np.float32)
                                for label in tqdm(train_labels, file=tqdm_out)]
        else:
            train_classes = [np.stack((get_mask(label), get_mask(label)>0), axis=0).astype(np.float32)
                                for label in tqdm(train_labels, file=tqdm_out)]
        if run_test:
            ...
            if self.nclasses==3:
                ...
                test_classes = [np.stack((get_mask(label), get_mask(label)>0, utils.distance_to_boundary(get_mask(label))), axis=0).astype(np.float32)
                                    for label in tqdm(test_labels, file=tqdm_out)]
            else:
                test_classes = [np.stack((get_mask(label), get_mask(label)>0), axis=0).astype(np.float32)
                                    for label in tqdm(test_labels, file=tqdm_out)]
        else:
            test_classes = None
    ...
    def _train_net(self, train_data, train_labels, 
        ...
        if isinstance(learning_rate, (list, np.ndarray)):
            ...
            epochs_left = max(0, self.n_epochs - 10)
            if epochs_left > 0:
                cosine_lr = self.learning_rate_const * 0.5 * (1 + np.cos(np.pi * np.arange(epochs_left) / epochs_left))
